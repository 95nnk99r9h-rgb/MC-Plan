...
Das Repository enthält **keinen** GitHub-Actions-Workflow. Veröffentlicht wird über die Einstellung
**Settings → Pages → Build and deployment → Source: „Deploy from a branch“** mit dem Arbeitszweig
und dem Ordner `/ (root)`.

GitHub liefert dabei den Zweig selbst aus. Die `index.html` im Stamm ist die Einstiegsdatei für die
Entwicklung und verweist auf `/src/main.tsx`, das ein Browser nicht ausführen kann. Ihr kleines
Skript wechselt deshalb nach 1,5 Sekunden auf **`app/`** – dort liegt ein **eingecheckter Build**.
Die Seite erscheint unter `https://<konto>.github.io/<repository>/` bzw. direkt unter
`…/<repository>/app/`.

**Nach jeder Änderung muss `app/` neu gebaut werden**, sonst zeigt Pages den alten Stand:

```bash
npm run build
rm -rf app && cp -r dist app
git add app && git commit -m "app/ neu gebaut"
```
...
Unterverzeichnis weiß. Die Navigation arbeitet mit Hash-Adressen (`#/planlauf/fristen`), daher
